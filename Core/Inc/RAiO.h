...
#define XPCLK_INV  0  // 0:no inversion, 1:inversion

#define HPW       2        // HSYNC Pulse Width[HPW]
#define HND       16        // Horizontal Non-Display Period[HND], Back porch
#define HDW       800       // Horizontal display width[HDW]
#define HST       44        // HSYNC Start Position[HST]

#define VPW       2         // VSYNC Pulse Width[VPW]
#define VND       5         // Vertical Non-Display Period[VND], Back porch
#define VDH       480       // Vertical Display Height[VDH]
#define VST       43        // VSYNC Start Position[VST]

#define RA8876_LVDS_FORMAT  0    // 0:Format1(VESA format), 1:Format2 =(JEIDA format)
...
// uint16_t powerSavingStatus(void);
...
//
 ...
 void setTextParameter2(uint16_t align, uint16_t chroma_key, uint16_t width_enlarge, uint16_t height_enlarge);//cdh
//
// void genitopCharacterRomParameter(uint16_t scs_select, uint16_t clk_div, uint16_t rom_select, uint16_t character_select, uint16_t gt_width);//b7h,bbh,ceh,cfh
//
 void putString(uint16_t x0,uint16_t y0, char *str);
 void  putStringData(char *str);
// void putDec(uint16_t x0,uint16_t y0,uint16_t vaule,uint16_t len,const char *flag);
...
// void bteMpuWriteColorExpansionWithChromaKey(uint16_t des_addr,uint16_t des_image_width, uint16_t des_x,uint16_t des_y,uint16_t width,uint16_t height,
//                                             uint16_t foreground_color,uint16_t background_color,const unsigned char *data);
// void bteMpuWriteColorExpansionWithChromaKey(uint16_t des_addr,uint16_t des_image_width, uint16_t des_x,uint16_t des_y,
...
// void setSerialFlash4BytesMode(uint16_t scs_select);
 void dma_24bitAddressBlockMode(uint16_t scs_selct,uint16_t clk_div,uint16_t x0,uint16_t y0,uint16_t width,uint16_t height,uint16_t picture_width,uint32_t addr);
// void dma_32bitAddressBlockMode(uint16_t scs_selct,uint16_t clk_div,uint16_t x0,uint16_t y0,uint16_t width,uint16_t height,uint16_t picture_width,uint16_t addr);
//
